...
# modal for generation of KM curves
genBSModal <- function(id, title, trigger, size) {
  b <- bsModal(id = id, title = title, trigger = trigger, size = size)
  
  # set modal header
  header <- b$children[[1]]$children[[1]]$children[[1]]$children
  header <- list(header[[1]], 
                 ...
                 bsButton("genBtn", "Generate KM Plot", icon = NULL, style = "success",
                          ...
                          value = FALSE),
                 header[[2]])
  b$children[[1]]$children[[1]]$children[[1]]$children <- header
  
  # set modal body
  b$children[[1]]$children[[1]]$children[[2]] <- list(
